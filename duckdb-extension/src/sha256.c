...
// Computes SHA-256 of a buffer or file, outputs lowercase hex string.
#include "sha256.h"
#include <stdio.h>
#include <string.h>
...
void sha256_buffer(const uint8_t *data, size_t len, char out_hex[ILI_SHA256_HEX_SIZE]) {
    ...
    for (int i = 0; i < SHA256_DIGEST_SIZE; i++) {
        out_hex[i * 2]     = "0123456789abcdef"[hash[i] >> 4];
        out_hex[i * 2 + 1] = "0123456789abcdef"[hash[i] & 0xf];
    }
